...
/**
 * Overrides Nest's built-in `service` schematic: the spec is always
 * generated (a team rule that survives `--no-spec`), and the generated
 * service gets a `Logger` field.
 */
export function teamServiceOverride(options: Schema): Rule {
  return (tree, context) => {
    const filesBefore = listFiles(tree);
    const className = `${strings.classify(options.name)}Service`;

    return chain([
      externalSchematic('@nestjs/schematics', 'service', { ...options, spec: true }),
      (tree2: Tree) => {
        for (const file of listFiles(tree2)) {
          if (!filesBefore.has(file) && file.endsWith('.service.ts')) {
